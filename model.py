...
class Encoder(nn.Module):
    def __init__(self, input_size, embed_size, hidden_size,
                 n_layers=1, dropout=0.5, vectors = None):
...
class Seq2Seq(nn.Module):
    ...
    def predict(self, src, MAX_LEN, sos, eos):
        ...
        vocab_size = self.decoder.output_size
        if cuda:
            outputs = Variable(torch.zeros(max_len, 1, vocab_size)).cuda()
        else:
            outputs = Variable(torch.zeros(max_len, 1, vocab_size))
        src = src.view(-1, 1)
        encoder_output, hidden = self.encoder(src)
        hidden = hidden[:self.decoder.n_layers]
        if cuda:
            output = Variable(torch.LongTensor([sos])).cuda().view(1,1)  # sos
        else:
            output = Variable(torch.LongTensor([sos])).view(1, 1)
        predict.append(sos)
        for t in range(1, max_len):
            output, hidden, attn_weights = self.decoder(
                    output, hidden, encoder_output)
